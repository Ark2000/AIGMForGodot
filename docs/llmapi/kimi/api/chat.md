# Content from https://platform.moonshot.cn/docs/api/chat
...
注意`image_url`和`video_url`的url字段支持 base64 格式和`ms:// `格式，详见 [使用 Kimi 视觉模型（Vision）](https://platform.moonshot.cn/docs/guide/use-kimi-vision-model)
...
[使用手册](https://platform.moonshot.cn/docs/introduction) [Tool Use](https://platform.moonshot.cn/docs/api/tool-use)