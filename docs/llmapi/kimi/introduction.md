# Content from https://platform.moonshot.cn/docs/introduction
...
你可以使用我们的 [List Models API](https://platform.moonshot.cn/docs/api/chat#list-models)来获取当前可用的模型列表。当前的我们支持的模型有：
...
`kimi-latest`已于 2026 年 1 月 28 日停止新用户使用，将不再维护和支持。请直接使用 Kimi 最新模型 [kimi-k2.5](https://platform.moonshot.cn/docs/guide/kimi-k2-5-quickstart)，以获得持续支持和更强推理能力。

`kimi-thinking-preview`已于 2025 年 11 月 11 日下线，不再维护和支持。建议直接升级至最新模型 [kimi-k2.5](https://platform.moonshot.cn/docs/guide/kimi-k2-5-quickstart)，以获得思考能力。

如需更多支持，请 [联系销售](https://platform.moonshot.cn/contact-sales)

# 使用指南

## 获取 API 密钥

你需要一个 API 密钥来使用我们的服务。你可以在我们的 [控制台](https://platform.moonshot.cn/console)中 [创建一个 API 密钥](https://platform.moonshot.cn/console/api-keys)。

## 发送请求

你可以使用我们的 Chat Completions API 来发送请求。你需要提供一个 API 密钥和一个模型名称。你可以选择是否使用默认的 max_tokens 参数，或者自定义 max_tokens 参数。可以参考 [API 文档](https://platform.moonshot.cn/docs/api/chat#%E5%8D%95%E8%BD%AE%E5%AF%B9%E8%AF%9D)中的调用方法。
...
[欢迎](https://platform.moonshot.cn/docs/overview) [Chat](https://platform.moonshot.cn/docs/api/chat)
