extends Node2D
## 休息设施：范围内按 [kbd]F[/kbd] 通过统一交互入口开始睡眠，结束时恢复精力。
##
## 契约（与 [group interactable_facility] 其它成员一致）：
## [method build_f_interact_entry]、[method open_player_interaction]。

signal interaction_state_changed

@export var display_name: String = "床"
@export var interact_radius: float = 88.0
@export var rest_duration_sec: float = 4.5
@export var energy_restore: float = 48.0
@export var preview_item_id: String = "misc_book"
...
func get_interact_label() -> String:
	return "休息 · %s" % display_name if not display_name.is_empty() else "休息"
...
func apply_rest_to_walker(walker: NekomimiWalker) -> void:
	if walker == null:
		return
	walker.add_energy(energy_restore)


func get_npc_action_duration_sec() -> float:
	return maxf(0.05, rest_duration_sec)


## 供 [NekomimiWalker] F 键列表与 [InteractablePicker] 使用。
